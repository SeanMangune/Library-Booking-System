...
class AuthController extends Controller
{
    ...
    public function register(Request $request, QcIdOcrVerifier $verifier)
    {
        $validated = $request->validate([
            ...
            'employee_category' => ['nullable', 'required_if:user_type,employee', 'string', 'max:100'],
            'course' => ['nullable', 'required_if:user_type,student', 'string', 'max:50'],
        ...
        if (! $verification['is_valid']) {
            ...
            return back()
                ->withInput()
                ->withErrors(['qcid_image' => $message]);
        }

        $user = null;

        DB::transaction(function () use ($request, $validated, $verification, &$user) {
            $user = User::create([
                'name' => $validated['name'],
        ...
        return $this->redirectFor($user)->with(
            'status',
            'Congratulations! Your Quezon City Citizen ID was verified successfully. Welcome to SmartSpace.'
        );
    }
