...
TEXT;
    ...
    public function test_staff_can_submit_qcid_registration_with_blank_email(): void
    {
        Storage::fake('public');
        /** @var User $staff */
...
Last Name, First Name, Middle Name
MANGUNE, SEAN MICHAEL CALINAWAN
Sex M    Date of Birth 10/01/2003    Civil Status SINGLE
Date Issued 02/16/2024    Valid Until 10/01/2034
19 A KING CONSTANTINE EXT
KINGSPOINT BAGBAG, QUEZON CITY
Cardholder Signature
005 000 01257479
TEXT;
        ...
        $response = $this->actingAs($staff)->post(route('qcid.registration.store'), [
            'full_name' => 'Sean Michael Mangune',
            'email' => '',
            'contact_number' => '09123456789',
            'qcid_number' => '005 000 01257479',
    ...
    public function test_qcid_verification_extracts_issued_and_validity_dates_from_mmddyyyy_layout(): void
    {
        /** @var User $user */
        $user = User::factory()->create();
...
MANGUNE, SEAN MICHAEL CALINAWAN
M 10/01/2003 SINGLE
02/16/2024 10/01/2034
DATE ISSUED VALID UNTIL
19 A KING CONSTANTINE EXT KINGSPOINT BAGBAG, QUEZON CITY
005 000 01257479
TEXT;

        $response = $this->actingAs($user)->postJson(route('qcid.verify'), [
            'ocr_text' => $ocrText,
            'user_name' => 'MANGUNE, SEAN MICHAEL CALINAWAN',
        ]);

        $response->assertOk();
        $response->assertJsonPath('success', true);
        $response->assertJsonPath('verification.date_issued', '2024/02/16');
        $response->assertJsonPath('verification.valid_until', '2034/10/01');
    }
}
