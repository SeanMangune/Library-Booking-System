        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="bg-gradient-to-r from-indigo-600 via-violet-600 to-purple-600 px-6 py-7">
                <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                    <div class="max-w-3xl">
                        <div
                            class="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-indigo-50">
                            User verification portal
                        </div>
                        <h1 class="mt-4 text-3xl font-extrabold tracking-tight text-white">Register your QC ID for account
                            verification</h1>
                        <p class="mt-2 text-sm text-indigo-100/95">Upload your Quezon City Citizen ID, review the detected
                            details, and submit your registration for verification. Only QC IDs are accepted.</p>
                    </div>
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-3 lg:min-w-[420px]">
                        ...
                        <div class="rounded-2xl border border-white/15 bg-white/10 px-4 py-4 text-white backdrop-blur-sm">
                            ...
                            <p class="mt-2 text-lg font-bold" x-text="verification?.is_valid
                                   ? 'QC Citizen ID'
            ...
            <div class="p-6 lg:p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
                    <div class="xl:col-span-2 space-y-6">
                        <form method="POST" action="{{ route('qcid.registration.store') }}" enctype="multipart/form-data"
                            class="space-y-6">
                            @csrf
                            ...
                            <div class="bg-gray-50 rounded-2xl border border-gray-200 p-5">
                                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600">Use a bright, clear photo where the ID text is
                                            readable.</p>
                                    </div>
                                    <button type="button" @click="reprocess()"
                                        class="inline-flex items-center justify-center gap-2 rounded-xl border border-indigo-200 bg-white px-4 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-50 transition-colors">
                                        Re-read QC ID
                                    </button>
                                </div>
                                ...
                                <div class="mt-5 grid grid-cols-1 gap-5 lg:grid-cols-[1.1fr_0.9fr]">
                                    <div class="rounded-2xl border-2 border-dashed border-indigo-200 bg-white p-5">
                                        <label for="qcid_image"
                                            class="flex cursor-pointer flex-col items-center justify-center text-center">
                                            <div
                                                class="flex h-16 w-16 items-center justify-center rounded-2xl bg-indigo-100 text-indigo-600">
                                                <i
                                                    class="h-8 w-8 fa-icon fa-solid fa-cloud-arrow-up text-3xl leading-none"></i>
                                            </div>
                                            <p class="mt-4 text-sm font-semibold text-gray-900">Drop your QC ID image here
                                                or click to browse</p>
                                            <p class="mt-1 text-xs text-gray-500">Accepted: JPG, PNG, WEBP up to 25 MB</p>
                                        </label>
                                        <input id="qcid_image" name="qcid_image" type="file"
                                            accept="image/png,image/jpeg,image/jpg,image/webp" class="sr-only"
                                            @change="handleFile($event)" required>

                                        <template x-if="imagePreview">
                                            <div class="mt-5 overflow-hidden rounded-2xl border border-gray-200 bg-gray-50">
                                                <img :src="imagePreview" alt="QC ID preview"
                                    ...
                                    <div class="space-y-4">
                                        <div x-show="isProcessing" x-cloak
                                            class="rounded-2xl border border-teal-200 bg-teal-50 px-4 py-4">
                                            <div class="flex items-center justify-between gap-4">
                                                ...
                                                <div class="text-lg font-extrabold text-teal-700"
                                                    x-text="Math.round(progress) + '%' "></div>
                                            </div>
                                            <div class="mt-3 h-2 rounded-full bg-teal-100 overflow-hidden">
                                                <div class="h-full rounded-full bg-gradient-to-r from-teal-500 to-emerald-500 transition-all duration-200"
                                        ...
                                        <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                                            <div class="flex items-center justify-between gap-3">
                                                <div>
                                                    ...
                                                    <p class="text-xs text-gray-500">Detected details from the uploaded
                                                        card.</p>
                                                </div>
                                                <span
                            ...
                            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                                <div class="flex items-center justify-between gap-3">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600">Confirm the captured details and add any missing
                                            information before submission.</p>
                                    </div>
                                    <div
                                        class="hidden sm:flex items-center gap-2 rounded-xl bg-indigo-50 px-3 py-2 text-xs font-semibold text-indigo-700">
                                ...
                                <div class="mt-5 grid grid-cols-1 gap-5 md:grid-cols-2">
                                    <div>
                                        ...
                                        <input name="full_name" x-model="form.full_name"
                                            value="{{ old('full_name', $registration?->full_name ?? $user->name) }}"
                                            type="text" required
                                            class="w-full rounded-xl border border-gray-300 px-4 py-2.5 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Email address</label>
                                        <input name="email" x-model="form.email" value="{{ $initialRegistrationEmail }}"
                                            type="email" :required="!isStaffUser"
                                            :placeholder="isStaffUser ? 'For student accounts only' : 'name@example.com'"
                                            class="w-full rounded-xl border border-gray-300 px-4 py-2.5 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Mobile number</label>
                                        <input name="contact_number" x-model="form.contact_number"
                                            value="{{ old('contact_number', $registration?->contact_number) }}" type="text"
                                            class="w-full rounded-xl border border-gray-300 px-4 py-2.5 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500"
                                    ...
                                    <div>
                                        <div class="mb-1 flex items-center justify-between gap-2">
                                            ...
                                            <button type="button" @click="openConfidenceModal('qcid_number')"
                                                class="inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-semibold"
                                                :class="confidenceBadgeClass('qcid_number')"
                                                x-text="confidenceLabel('qcid_number')"></button>
                                        </div>
                                        <input name="qcid_number" x-model="form.qcid_number"
                                            value="{{ old('qcid_number', $registration?->qcid_number) }}" type="text"
                                            class="w-full rounded-xl border border-gray-300 px-4 py-2.5 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Sex</label>
                                        <input name="sex" x-model="form.sex" value="{{ old('sex', $registration?->sex) }}"
                                            type="text"
                                            class="w-full rounded-xl border border-gray-300 px-4 py-2.5 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Civil status</label>
                                        <input name="civil_status" x-model="form.civil_status"
                                            value="{{ old('civil_status', $registration?->civil_status) }}" type="text"
                                            class="w-full rounded-xl border border-gray-300 px-4 py-2.5 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Date of birth</label>
                                        <input name="date_of_birth" x-model="form.date_of_birth"
                                            value="{{ old('date_of_birth', optional($registration?->date_of_birth)->format('Y-m-d')) }}"
                                            type="date"
                                            class="w-full rounded-xl border border-gray-300 px-4 py-2.5 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Date issued</label>
                                        <input name="date_issued" x-model="form.date_issued"
                                            value="{{ old('date_issued', optional($registration?->date_issued)->format('Y-m-d')) }}"
                                            type="date"
                                            class="w-full rounded-xl border border-gray-300 px-4 py-2.5 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500">
                                    </div>
                                    <div class="md:col-span-2">
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Valid until</label>
                                        <input name="valid_until" x-model="form.valid_until"
                                            value="{{ old('valid_until', optional($registration?->valid_until)->format('Y-m-d')) }}"
                                            type="date"
                                            class="w-full rounded-xl border border-gray-300 px-4 py-2.5 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500">
                                    </div>
                                    <div class="md:col-span-2">
                                        <div class="mb-1 flex items-center justify-between gap-2">
                                            ...
                                            <button type="button" @click="openConfidenceModal('address')"
                                                class="inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-semibold"
                                                :class="confidenceBadgeClass('address')"
                                                x-text="confidenceLabel('address')"></button>
                                        </div>
                                        <textarea name="address" x-model="form.address" rows="3"
                                            class="w-full rounded-xl border border-gray-300 px-4 py-2.5 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500">{{ old('address', $registration?->address) }}</textarea>
                                    </div>
                                </div>

                                <div
                                    class="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-t border-gray-100 pt-5">
                                    <p class="text-sm text-gray-500">Submitting will set your QC ID registration status to
                                        pending review.</p>
                                    <button type="submit" :disabled="!verification?.is_valid || isProcessing"
                                        class="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-600/25 transition-all hover:from-indigo-700 hover:to-violet-700 disabled:cursor-not-allowed disabled:opacity-60">
                    ...
                    <div class="space-y-6">
                        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                            ...
                            <div class="mt-4 space-y-4">
                                <div class="flex gap-3">
                                    <div
                                        class="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-100 text-indigo-700 font-bold">
                                        1</div>
                                    <div>
                                        <p class="font-semibold text-gray-900">Upload your QC ID</p>
                                        <p class="text-sm text-gray-600">The system reads the image and checks QC-specific
                                            markers.</p>
                                    </div>
                                </div>
                                <div class="flex gap-3">
                                    <div
                                        class="flex h-9 w-9 items-center justify-center rounded-xl bg-violet-100 text-violet-700 font-bold">
                                        2</div>
                                    <div>
                                        <p class="font-semibold text-gray-900">Review captured data</p>
                                        <p class="text-sm text-gray-600">Detected fields are prefilled to help avoid manual
                                            encoding errors.</p>
                                    </div>
                                </div>
                                <div class="flex gap-3">
                                    <div
                                        class="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-100 text-emerald-700 font-bold">
                                        3</div>
                                    <div>
                                        <p class="font-semibold text-gray-900">Submit for verification</p>
                                        <p class="text-sm text-gray-600">Your registration is stored as pending so it can be
                                            reviewed later.</p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div
                            class="bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 rounded-2xl border border-slate-800 shadow-sm overflow-hidden">
                            ...
                            <div class="p-5 space-y-4 text-sm text-indigo-100">
                                <div class="flex items-center justify-between gap-4">
                                    ...
                                    <span
                                        class="font-semibold text-white">{{ $registration?->submitted_at?->format('M d, Y h:i A') ?? 'No submission yet' }}</span>
                                </div>
                                <div class="flex items-start justify-between gap-4">
                                ...
                                <div class="flex items-start justify-between gap-4">
                                    ...
                                    <span
                                        class="text-right font-semibold text-white">{{ $registration?->verification_notes ?? 'Awaiting review.' }}</span>
                                </div>

                                @if($registration?->qcid_image_path)
                                    <div class="rounded-2xl overflow-hidden border border-white/10 bg-white/5">
                                        <img src="{{ asset('storage/' . $registration->qcid_image_path) }}" alt="Stored QC ID"
