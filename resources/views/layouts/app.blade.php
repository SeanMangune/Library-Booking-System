                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" class="flex items-center gap-3 text-sm font-medium text-gray-700 hover:text-gray-900 p-1.5 rounded-lg hover:bg-gray-100 transition-colors">
                                @php
                                    $authUser = auth()->user() ?? \App\Models\User::query()->orderBy('id')->first();
                                    $initials = $authUser
                                        ? collect(explode(' ', trim($authUser->name)))->filter()->map(fn($p) => mb_substr($p, 0, 1))->take(2)->implode('')
                                        : 'U';
                                @endphp
                                ...
                                <div class="hidden sm:block text-left">
                                    <p class="font-semibold text-gray-800">{{ $authUser?->name ?? 'User' }}</p>
                            ...
                            <div x-show="open" @click.away="open = false" x-cloak
                                ...
                                <div class="px-4 py-3 border-b border-gray-100">
                                    <p class="text-sm font-semibold text-gray-800">{{ $authUser?->name ?? 'User' }}</p>
